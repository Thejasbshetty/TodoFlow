...
const Todos = () => {
  ...
  const fetchTodos = async () => {
    try {
      ...
      const response = await axios.get('https://todos-backend-i1uv.onrender.com/api/todos', {
        headers: {
  ...
  const fetchUserDetails = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('https://todos-backend-i1uv.onrender.com/api/auth/user', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleMarkAsDone = async (id, currentStatus) => {
    try {
      ...
      await axios.put(
        `https://todos-backend-i1uv.onrender.com/api/todos/${id}`,
